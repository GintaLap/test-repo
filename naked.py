...
with open('.log_worker.yaml','r') as stream:
        log_config = yamal.safe_load(stream)
...
def mysql_insert_ast_into_db(create_date, hazardous, name, url, diam_min, diam_max, ts, dt_utc, dt_local, speed, distance, ast_id):
        ...
        try:
                cursor = connection.cursor()
                result  = cursor.execute( "INSERT INTO `ast_daily` (`create_date`, `hazardous`, `name`, `url`, `diam_min`, `diam_max`, `ts`, `dt_utc`, `dt_local`, `speed`, `distance`, `ast_id`) VALUES ('" + >
                connection.commit()
        except Error as e :
                logger.error( "INSERT INTO `ast_daily` (`create_date`, `hazardous`, `name`, `url`, `diam_min`, `diam_max`, `ts`, `dt_utc`, `dt_local`, `speed`, `distance`, `ast_id`) VALUES ('" + str(create_d>
...
#If html technical error 200 we good
	if r.status_code == 200:
#reading Json with response 
		json_data = json.loads(r.text)
#decalring safe and dangerous asteroids lists 
		ast_safe = []
		ast_hazardous = []
#Counting asteroids for the day
		if 'element_count' in json_data:
			ast_count = int(json_data['element_count'])
			logger.info("Asteroid count today: " + str(ast_count))
#If count more than 0 gathering the data about them
			if ast_count > 0:
				for val in json_data['near_earth_objects'][request_date]:
					if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val:
						...
						if 'kilometers' in val['estimated_diameter']:
							...
							tmp_ast_diam_min = -1
							tmp_ast_diam_max = -1
#Marking potentialy dangerous asteroids
						tmp_ast_hazardous = val['is_potentially_hazardous_asteroid']
#Calculating other parameters from response
						if len(val['close_approach_data']) > 0:
							if 'epoch_date_close_approach' and 'relative_velocity' and 'miss_distance' in val['close_approach_data'][0]:
								tmp_ast_close_appr_ts = int(val['close_approach_data'][0]['epoch_date_close_approach']/1000)
								tmp_ast_close_appr_dt_utc = datetime.utcfromtimestamp(tmp_ast_close_appr_ts).strftime('%Y-%m-%d %H:%M:%S')
								tmp_ast_close_appr_dt = datetime.fromtimestamp(tmp_ast_close_appr_ts).strftime('%Y-%m-%d %H:%M:%S')
#Stating speed of the asteroids
							...
							tmp_ast_speed = -1
							tmp_ast_miss_dist = -1
#Printing out ino about asteroids
						logger.info("------------------------------------------------------- >>")
...
#Printing out that we won't die 
			else:
				logger.info("No asteroids are going to hit earth today")
#Categorizing asteroids and providing the data
		logger.info("Hazardous asteorids: " + str(len(ast_hazardous)) + " | Safe asteroids: " + str(len(ast_safe)))
#If more than 0 asteroids  
		if len(ast_hazardous) > 0:
#Thank checking other keys 
			ast_hazardous.sort(key = lambda x: x[4], reverse=False)
#And determining that things are bad
...
#Sorting asteroidsand providing distance info 
